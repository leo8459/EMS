            ...
            <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
                ...
                @foreach ($selectedAdmisionesData as $index => $data)
                    <div class="border rounded p-3 mb-3">
                        <h6>Envío interno: {{ $data['codigo'] }}</h6>
                        <!-- Peso EMS -->
...
<div class="border rounded p-3 mb-3">
    ...
    <div class="form-group">
        ...
        <input type="number" step="0.01"
               class="form-control"
               id="pesoExterno-{{ $index }}"
               wire:model="selectedSolicitudesExternasData.{{ $index }}.peso_o">
    </div>

    <!-- Observación -->
    <div class="form-group">
        <label for="observacionExterna-{{ $index }}">Observación (opcional)</label>
        <textarea class="form-control"
                  id="observacionExterna-{{ $index }}"
                  wire:model="selectedSolicitudesExternasData.{{ $index }}.observacion"></textarea>
    </div>

    <!-- Combo Box: Notificación -->
    <div class="form-group">
        <label for="notificacionExterna-{{ $index }}">Notificación</label>
        <select class="form-control"
                id="notificacionExterna-{{ $index }}"
                wire:model="selectedSolicitudesExternasData.{{ $index }}.notificacion">
            <option value="">Seleccione...</option>
            <option value="FALTANTE">FALTANTE</option>
            <option value="SOBRANTE">SOBRANTE</option>
            <option value="MALENCAMINADO">MALENCAMINADO</option>
